...
public:
	...
	static NiNodePtr CreateBoundingBox(NiNode* Parent)
	{
		NiNodePtr BoundingBox = FiestaOnlineTool::GetBoundingBox();
	...
	static NiNodePtr LoadBaseNode() 
	{
		NiNodePtr Node;
		PgUtil::LoadNodeNifFile(PgUtil::PathFromApplicationFolder(".\\FiestaOnlineTool\\BoundingBox.nif").c_str(), &Node, NULL);
		return Node;
	}
};